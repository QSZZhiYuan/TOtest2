...
public class NightwardenAdaptRegistry {
    ...
    public static boolean isMagicAdaptable(DamageSource source, ResourceLocation damageType) {
        return damageType != null && (MAGIC_ADAPTATION_TYPES.contains(damageType) || source.m_269533_(TravelopticsTags.NIGHTWARDEN_MAGIC_ADAPT));
    }

    public static boolean isAllowedInMeleePhase(DamageSource source, ResourceLocation damageType) {
        return damageType != null && (MELEE_PHASE_DAMAGE_TYPES.contains(damageType) || source.m_269533_(TravelopticsTags.NIGHTWARDEN_MELEE_PHASE_ACCEPT));
    }
}
