...
extends Goal {
    ...
    public void m_8037_() {
        if (this.target != null) {
            boolean currentlyHasLineOfSight = this.mob.m_21574_().m_148306_((Entity)this.target);
    ...
    protected void castSpellWhenOutOfSight() {
        if (!this.spellCastingMob.isCasting() && !this.spellCastingMob.isDrinkingPotion()) {
            ...
            if (!spell.shouldAIStopCasting(spellLevel = Math.max(spellLevel, 1), (Mob)this.mob, this.target)) {
                ...
                this.cooldownTime = Mth.m_14107_((double)Mth.m_14139_((double)this.mob.m_217043_().m_188500_(), (double)this.attackIntervalMin, (double)this.attackIntervalMax));
            }
        }
    }
    ...
    protected AbstractSpell getNextSpellType() {
        ...
        if (!this.drinksPotions || spellList != this.supportSpells || !this.supportSpells.isEmpty() && !(this.mob.m_217043_().m_188501_() < 0.5f)) {
            return (AbstractSpell)spellList.get(this.mob.m_217043_().m_188503_(spellList.size()));
        }
