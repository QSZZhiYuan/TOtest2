...
public class SummonCheckHelper {
    public static boolean hasActiveSummons(Player player, double range, Set<Class<? extends MagicSummon>> summonRegistry) {
        ...
        List summons = level2.m_6443_(LivingEntity.class, player.m_20191_().m_82400_(range), entity -> {
            MagicSummon magicSummon;
            return SummonCheckHelper.isRegisteredSummon(entity, summonRegistry) && entity instanceof MagicSummon && (magicSummon = (MagicSummon)entity).getSummoner() != null && magicSummon.getSummoner().m_20148_().equals(player.m_20148_());
        });
