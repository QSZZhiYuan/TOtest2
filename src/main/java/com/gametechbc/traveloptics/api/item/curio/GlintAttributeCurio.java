...
public class GlintAttributeCurio
extends CurioBaseItem {
    private final Multimap<Attribute, AttributeModifier> attributeMap = HashMultimap.create();

    public GlintAttributeCurio(Item.Properties properties, Map<Attribute, CustomAttribute> attributes) {
        super(properties);
        for (Map.Entry<Attribute, CustomAttribute> entry : attributes.entrySet()) {
            AttributeModifier.Operation operation = GlintAttributeCurio.parseOperation(entry.getValue().getOperation());
