...
public class StellothornLevelThreeItem
extends StellothornItem
implements UniqueItem {
    private static ItemDisplayContext transformType;

    public StellothornLevelThreeItem(SpellDataRegistryHolder[] imbuedSpells) {
        super(imbuedSpells);
    }
    ...
    protected void spawnStellothornProjectile(Level level, Player player, ItemStack stack, float chargePower) {
        if (!level.f_46443_) {
            ...
            double eldritchPower = player.m_21133_((Attribute)AttributeRegistry.ELDRITCH_SPELL_POWER.get());
            double effectivePower = Math.max(0.0, eldritchPower - 1.0);
            float baseCloneDamage = 6.0f;
            float scaledCloneDamage = (float)((double)baseCloneDamage + 2.5 * effectivePower + 1.5 * Math.sqrt(effectivePower));
            spearEntity.setCloneDamage(scaledCloneDamage);
    ...
    public void m_7373_(ItemStack stack, Level world, List<Component> tooltip, TooltipFlag flag) {
        ...
        if (Screen.m_96638_()) {
            ...
            tooltip.add((Component)Component.m_237115_((String)"item.traveloptics.stellothorn.evo_two.active.tooltip"));
            tooltip.add((Component)Component.m_237115_((String)"item.traveloptics.stellothorn.evo_three.active.tooltip"));
        } else {
